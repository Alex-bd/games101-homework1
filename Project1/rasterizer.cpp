...
void rst::rasterizer::draw_line(Eigen::Vector3f begin, Eigen::Vector3f end) //直线扫描划线，中点画线算法
{
    auto x1 = begin.x();
    auto y1 = begin.y();
    auto x2 = end.x();
    ...
    if(dy1<=dx1)//如果线段斜率的绝对值小于等于1就执行下列代码，这样区分是因为像素点坐标是整数，
				//而如果斜率的绝对值小于1，在对线段进行采样时，每次x坐标加1，y坐标要么是加1，要么是减1，要么不变，简化了计算，
				//而当斜率的绝对值大于1时，斜率的倒数的绝对值就小于1，就可以每次对y坐标加1,也是一样的效果
    {
        if(dx>=0)
        {
            x=x1;
...
{
    return Eigen::Vector4f(v3.x(), v3.y(), v3.z(), w);   //三维变四维
}
...
void rst::rasterizer::set_pixel(const Eigen::Vector3f& point, const Eigen::Vector3f& color)//将屏幕像素点 (x, y) 设 为 (r, g, b) 的颜色，并写入相应的帧缓冲区位置
{
    ...
    auto ind = (height-point.y())*width + point.x();//这一步是根据坐标求像素在帧缓冲区的序号
